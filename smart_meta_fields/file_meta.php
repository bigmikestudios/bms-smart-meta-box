...
function smart_meta_attachment_fields_to_edit($form_fields, $post) {
	...
	$form_fields["custom_field_template"]["html"] = '<a href="javascript:void(0);" onclick="parent.jQuery(parent.smb_file_field).val(\''.$post->ID.'\');
																							parent.jQuery(parent.smb_file_label).html(\''.$my_image_tag.$post->post_title.'\'); 
																							top.tb_remove();">'.__('Use this', 'smart-meta').'</a>';
	return $form_fields;
}
